...
return mktime(0, 0, 0, $partes[1], $partes[0], $partes[2]);
		...
		$dadosboleto["texto_boleto"] = $linha['texto_boleto'];
...
if(isset($_GET['id_venda'])){
		...
		if($dias <= 0){	
			...
			$jurosumdia = round($valor_doc * ($juros / 100),2);
			$valordojuro = $dias*$jurosumdia;

		$valor_boleto = $valor_doc + $valordojuro + $valormulta;
...
/////////////////////////////Para impressão em lote//////////////////			
	...
	foreach($_POST['ids'] as $key => $id_fatura){	
		...
		if (!empty($numero)){
			...
			$dadosboleto["endereco1"] = "$endereco, S/N $Ccomplemento - $bairro";
		}
