...
<strong>OBS: Será cobrado Juros mais multa de todos os pagamentos em atraso.</strong>
</div>
<?php 
...
$sql_1 = "SELECT * ,date_format(data_venci, '%d/%m/%Y') AS data FROM faturas WHERE id_cliente = '$id_cliente' AND id_venda LIKE '%$pesquisar%' AND motivo_baixa != 'Cancelamento do boleto'";
}
...
$sql_1 = "SELECT * ,date_format(data_venci, '%d/%m/%Y') AS data FROM faturas WHERE id_cliente = '$id_cliente' AND motivo_baixa != 'Cancelamento do boleto' ORDER BY id_venda DESC";	
}
...
<tbody>
  <tr>
    ...
    <td width="57"  align="center" bgcolor="#0490fc"><span class="fontebranca">Nº Doc</span></td>
    ...
    <td width="224" colspan="2" align="center" bgcolor="#0490fc"><span class="fontebranca">Imprimir</span></td>
    </tr>
</tbody>
<?php
...
if ($array['motivo_baixa'] == 'CANCELADA' || $array['motivo_baixa'] == 'Cancelamento do boleto'){
}else {
...
?>
  <tr>
	...
	if($array['situacao'] != 'B'){
	?>
    <a href="<?php echo "../boleto/boleto_itau.php?id_venda=".$array['id_venda']; ?>" target="_blank">
    <img src="img/pagboleto.png" width="38" height="26" title="Pagar com Boleto Bancário" /></a>
    &nbsp;|&nbsp;
    <a href="<?php echo "../boleto/boleto_itau_pdf.php?id_venda=".$array['id_venda']; ?>" target="_blank">
    <img src="img/pdf-icone.png" height="26" title="Baixar PDF" /></a>
