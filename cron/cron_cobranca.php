...
$ups = mysql_query("UPDATE faturas SET control_email = 0 WHERE control_email = 5");
...
$sql = mysql_query("SELECT *,date_format(data_venci, '%d/%m/%Y') AS data FROM faturas WHERE situacao = 'V' AND control_email < 5") or die(mysql_error());
...
if ($enviado) {
  ...
  $up = mysql_query("UPDATE faturas SET control_email = control_email + 1 WHERE id_venda = $idfatura");
} else {
