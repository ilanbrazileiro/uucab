...
<form action="inicio.php?pg=fatvencida<?php echo $pags.$part; ?>" method="post" enctype="multipart/form-data">
<span class="avisos">&nbsp;*Pesquise pelo DIRETOR ou numero do documento</span><br/>
  ...
  <div style="float:left;width:300px">
	...
	<form action="inicio.php?pg=fatvencida<?php echo $pags.$part; ?>" method="post" enctype="multipart/form-data" name="formu" id="formu" onSubmit="return datas(this);">
...
$sql_1 = "SELECT f.*, c.*, date_format(data_venci, '%d/%m/%Y') AS datav, f.valor AS v FROM faturas AS f INNER JOIN cliente AS c ON f.id_cliente = c.id_cliente WHERE f.situacao ='V' AND (c.dir_culto LIKE '%$pesquisar%' OR f.id_venda LIKE '%$pesquisar%') ORDER BY ".$campo." ".$ordem;

} elseif(isset($_POST['matric']) && $_POST['matric'] != ""){
$pesquisar = $_POST['matric'];
$filtro = "matric:'".$_POST['matric']."'";
$sql_1 = "SELECT f.*, c.*, date_format(data_venci, '%d/%m/%Y') AS datav, f.valor AS v FROM faturas AS f INNER JOIN cliente AS c ON f.id_cliente = c.id_cliente WHERE f.situacao ='V' AND (c.matricula LIKE '%$pesquisar%')  ORDER BY matricula ASC";
...
$sql_1 = "SELECT f.*, c.*, date_format(data_venci, '%d/%m/%Y') AS datav, f.valor AS v FROM faturas AS f INNER JOIN cliente AS c ON f.id_cliente = c.id_cliente WHERE f.situacao ='V' AND (f.data_venci BETWEEN ('$datai') AND ('$dataf')) ORDER BY ".$campo." ".$ordem;	

}else{
$sql_1 = "SELECT f.*, c.*, date_format(data_venci, '%d/%m/%Y') AS datav, f.valor AS v FROM faturas AS f INNER JOIN cliente AS c ON f.id_cliente = c.id_cliente WHERE f.situacao ='V' ORDER BY ".$campo." ".$ordem;	
}
...
if ($pags>1){
echo "<a class=\"pag\" href=\"#\" onclick=\"sendPost('inicio.php?pg=fatvencida".$part."',{".$filtro."}, '1')\" target=\"_self\">&laquo;&laquo; Primeira</a> ";
}
?>
&nbsp;

<select name="p" onchange="sendPost('inicio.php?pg=fatvencida<?php echo $part;?>',{<?php echo $filtro; ?>}, this.value );" style="width: 50px;">          
...
echo "<a class=\"pag\" href=\"#\" onclick=\"sendPost('inicio.php?pg=fatvencida".$part."',{".$filtro."}, '".$pags."')\" target=\"_self\">Ultima &raquo;&raquo;</a> ";
}
?>
...
<tbody>
  <tr>
   ...
   <td width="40" bgcolor="#0490fc"><span class="fontebranca"><a style="color:#fff; text-decoration:none;" href="#" onclick="sendPost('inicio.php?pg=fatvencida&o=<?php echo $o;?>&campo=c.matricula',{<?php echo $filtro; ?>}, '1')" target="_self">Matr&nbsp;<i class="icon-arrow-down"></i></a></span></td>
    
    <td width="210" bgcolor="#0490fc"><span class="fontebranca"><a style="color:#fff; text-decoration:none;" href="#" onclick="sendPost('inicio.php?pg=fatvencida&o=<?php echo $o;?>&campo=c.dir_culto',{<?php echo $filtro; ?>}, '1')" target="_self">Diretor do Culto&nbsp;<i class="icon-arrow-down"></i></a></span></td>
    ...
    <td width="40" align="center" bgcolor="#0490fc"><span class="fontebranca"><a style="color:#fff; text-decoration:none;" href="#" onclick="sendPost('inicio.php?pg=fatvencida&o=<?php echo $o;?>&campo=f.id_venda',{<?php echo $filtro; ?>}, '1')" target="_self">Nº Doc&nbsp;<i class="icon-arrow-down"></i></a></span></td>
    
    <td width="55" align="center" bgcolor="#0490fc"><span class="fontebranca"><a style="color:#fff; text-decoration:none;" href="#" onclick="sendPost('inicio.php?pg=fatvencida&o=<?php echo $o;?>&campo=data_venci',{<?php echo $filtro; ?>}, '1')" target="_self">Venc.&nbsp;<i class="icon-arrow-down"></i></span></td>
...
?>
 ...
 <td align="center" valign="middle">
    ...
    <div class="btn-group">
    <a class="btn btn-default" href="php/reenviarmailatrazo.php?id_venda=<?php echo $array['id_venda'] ?>&idcliente=<?php echo $array['id_cliente'] ?><?php echo $vari ?>" title="Reenviar fatura por e-mail" style="color:#F60">
...
<?php } ?> 
	<tr>
        ...
        <?php 
		$sqlsoma = mysql_query("SELECT count(*) AS val FROM faturas WHERE situacao='V'");
