...
class RetornoCNAB400Conv6 extends RetornoCNAB400Base {
  ...
  protected function processarHeaderArquivo($linha) {
    $vlinha = parent::processarHeaderArquivo($linha);	
		...
		$vlinha["cod_retorno1"]           = substr($linha, 2,    1); //codigo do retorno 1
		$vlinha["cod_retorno2"]           = substr($linha, 109,    5); //codigo do retorno 2	
		$vlinha["dt_geracao"]	          = substr($linha, 095,  6);// datada geracao do arquivo																			
	...
	* @return array<mixed> Retorna um vetor contendo os dados dos campos da linha detalhe.*/
	protected function processarDetalhe($linha) {
		...
		$vlinha["tipo_registro"]        = substr($linha,  1,  1); //9  Identificação do registro transação
		$vlinha["c_ocorrencia"]        = substr($linha,  109,  2); //9  identificação da ocorrencia
		$vlinha["detalhe_ocorrencia"]     	= substr($linha, 110,   2); //detalhes da ocorrencia
		$vlinha["d_ocorrencia"]        = substr($linha,  111,  6); //9  data da ocorrencia
		$vlinha["d_credito"]        = substr($linha,  296,  6); //9  data do credito desta liquidação
		$vlinha["c_liquidacao"]        = substr($linha,  393,  2); //9  Nosso-Número
		
		
		$vlinha["nosso_numero"]        = substr($linha,  63,  11); //9  Nosso-Número
		$vlinha["banco"]     		= substr($linha, 166,   3);// codigo do banco
		$vlinha["ag_receb"]     	= substr($linha,  18,   4);// agencia que recebeu
		$vlinha["dv_receb"] 		= substr($linha,  22,   2);// 
		$vlinha["vencimento"]     	= substr($linha, 147,   6);//vencimento do boleto
		$vlinha["valor"]=$this->formataNumero(substr($linha, 153,  13)); //valor do boleto
		$vlinha["conta_receb"]     	= substr($linha, 24,   5);// conta que recebeu
		$vlinha["dac"]     			= substr($linha, 29,   1);// digito da conta que recebeu
		$vlinha["n_pagador"]     	= substr($linha, 325,   30);
	...
	* @return array Retorna um vetor associativo contendo os valores_linha processada.*/
	public function processarLinha($numLn, $linha) {
      ...
      $tipoLn = substr($linha,  1,  1);
		  if($tipoLn == RetornoCNAB400Conv6::HEADER_ARQUIVO) 
         $vlinha = $this->processarHeaderArquivo($linha);
      else if($tipoLn == RetornoCNAB400Conv6::DETALHE)
				 $vlinha = $this->processarDetalhe($linha);
		  else if($tipoLn == RetornoCNAB400Conv6::TRAILER_ARQUIVO)
